...
/**@Deprecated. This API converts the float value to an integer. The converted integer value is always the integer part of the specified float number (number before the decimal).*/
	...
	min: function () {
		return voltmx.math.findExtreme(false, arguments);
	},

	/**@Deprecated. This API returns the maximum value among the arguments.*/
	max: function () {
		return voltmx.math.findExtreme(true, arguments);
	},
...
/**The string Library has APIs that you can use to manipulate strings. The voltmx.string name space provides static string APIs. These static APIs augment the APIs in the Global string object which is available by default. For more information refer, https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/string.*/
voltmx.string = {
...
/**@Deprecated This API changes the lower case characters of the source string to upper case characters.*/
	...
	startsWith: function (sourcestring, comparestring, ignorecase) {
		return voltmx.string.matchEnds(arguments, false);
	},
	...
	endsWith: function (sourcestring, comparestring, ignorecase) {
		return voltmx.string.matchEnds(arguments, true);
	},
	...
	replace: function (s, f, rep) {
		...
		var exp1 = new RegExp(voltmx.string.escapeRegExp(f), "g");
	...
	containsChars: function (s, a) {
		...
		charstr = "[" + voltmx.string.escapeRegExp(charstr) + "]";
...
Note: This library is also available in JavaScript under the namespace voltmx.table. This is only for maintaining the backward compatibility for the application converted from Lua to JavaScript.*/
voltmx.table = {
/**@Deprecated This API extracts all the elements of a table and concatenates the elements to form a string. A separator can be specified which can be placed between concatenated elements.*/
	concat: function(inputtable, separator, startposition, endposition) {
	    ...
	    if (!isArgsError) {
	        ...
	        if (len == 0) 
	            return voltmx.table.returnResult("", isArgsError, isInternalError);
	        
	        if (isArgsError) 
	            return voltmx.table.returnResult(result, isArgsError, isInternalError);
	                    ...
	                    try {
	                        for (var i = begin; i < end; i++) {
								if(arguments[0][i] == null || arguments[0][i] instanceof Object) 
									return voltmx.table.returnResult(null, true, isInternalError);   
	       ...
	       // var arr = arguments[0];	
	        if (arguments.length > 2) {  
	            ...
	            if (typeof pos == "string" || isNaN(pos)) {                
	                return voltmx.table.returnResult(true, isInternalError);            
				}            
	            newItem = arguments[2];
	        }
	        else {
				...
				/*if(args[1] == null) // don't insert nil at the end of table -- lua std 
					return voltmx.table.returnResult(args[0], isArgsError, isInternalError);*/
	        }
			...
			if (pos >= 0) {
				if (arguments.length > 1 && arguments[1] != null) {
					...
					if (isNaN(arguments[1]) || arguments[1] > pos || arguments[1] < 0) {
						isArgsError = true;
						return voltmx.table.returnResult(result, isArgsError, false);
					}
				...
				/*for(var i=pos;i > 0;i--){
	    		  ...
	    		  return voltmx.table.returnResult(null, isArgsError, false);
		}
	    return voltmx.table.returnResult(result, isArgsError, false);
	},
	   ...
	   if (0 === arguments.length || !(arguments[0] instanceof Array)) {
	    ...
	    if (!isArgsError) {
			...
			if(arguments[0].length < 2){
				...
				return voltmx.table.returnResult(inputtable,false,false);
			}
	    ...
	    if (!isArgsError && !isInternalError)        
	   		 return voltmx.table.returnResult(pre, isArgsError, isInternalError);
		else 
			return voltmx.table.returnResult(true, isInternalError);
	},
/**@Deprecated This API filters the given table based on the provided criteria and returns a set of values that match the given criteria.*/
	filter: function(srctable, mapfunc) {
	    ...
	    return voltmx.table.returnResult(tgttable, isArgsError, isInternalError);
	},
	...
	map : function(srctable, mapfunc) {
	    ...
	    return voltmx.table.returnResult(srctable, isArgsError, isInternalError);
	...
	mapNew: function(srctable, mapfunc){
		...
		if(srctable == null)
			return voltmx.table.returnResult(null, isArgsError, isInternalError);
...
/**@Deprecated This API searches the given input table and returns the value at the specified key; if the key is a number, this API returns the value at the index.*/	
	get: function(srctable, key) {
	    ...
	    return voltmx.table.returnResult(result, isArgsError, isInternalError, invalidKey);
	},
/**@Deprecated Tests if the specified key is part of the table.*/	
	contains: function(srctable, key) {
	    ...
	    if (arguments.length < 2 || srctable instanceof Object === false || key == null) {        
	        return voltmx.table.returnResult(result, true, isInternalError);
	    }
...
return list[i], list[i+1], .... list[j].*/	
	...
	/*parsejson: function(args){
	    ...
	    try {
	        var tgttable = voltmx.utils.json2LuaTable(jsObj);
	    } 
	    catch (e) {
...
/**@Deprecated This API returns the current time as a string in hh:mm:ss format. The time is represented in 12 hour format.*/
	...
	voltmx.os.diffDateTime = function (time1, time2) {
	...
	voltmx.os.date = function () {
	    ...
	    if (0 === arguments.length) {
	        ...
	        result = voltmx.os.padZero(currentDate.getMonth() + 1) + "/" + voltmx.os.padZero(currentDate.getDate()) + "/" + voltmx.os.padZero(currentDate.getFullYear() % 100) + " " + timeStr.slice(0, timeStr.indexOf(" "));
	        return result;
	    }
	        ...
	        if (arguments[0].toLowerCase().indexOf("dd") != -1) {        
	            ...
	            return voltmx.os.formatdate(arguments[0], currentDate);            
	        }
	         else {
	                ...
	                if ('*' === arguments[0].charAt(index) && 't' === arguments[0].charAt(index + 1)) {
	                    ...
	                    result["yday"] = voltmx.os.getDayOfYear(day, mon, year);
	                    result["isdst"] = utc ? false : voltmx.os.checkForDst();
	                    return result;
	           	}
	           else 
	                    return null;
...
/**This API allows you to convert the given number to represent currency. At present, only USA currency is supported.*/
	...
	voltmx.os.compareDates = function (d1, d2, frmt){
		
		if(d1 == null || d2 == null || frmt == null || !voltmx.os.isvaliddate(d1,frmt) || !voltmx.os.isvaliddate(d2,frmt))
			return null 
			
		var date1 = voltmx.os.getDate(d1, frmt);
		var date2 = voltmx.os.getDate(d2, frmt);
...
/**@Deprecated This API adds or subtracts units (days, hours, minutes, month, or years) to the given date.*/
	...
	voltmx.os.addToDate = function (d1, frmt, unt, cnt){
		...
		if(!voltmx.os.isvaliddate(inputDate,fmt))
			return null;
		
		var dateObj = voltmx.os.getDate(inputDate, fmt);
		...
		if (dateObj) {
			...
			if(voltmx.os.isLeapYear([d1,fmt]) && dateObj.getMonth() >= 1 && (unit == "years" || (unit == "months" && (count == -12 || count == 12))) )
					dateObj.setDate(dateObj.getDate() - 1);
			
			return voltmx.os.formatdate(fmt,dateObj);
	...
	//os.isLeapYear(yyyy) 
	voltmx.os.isLeapYear = function (d1, frmt) {
		var year;
	    ...
	    if (typeof(d1) == "string" && typeof(frmt) == "string") {
			...
			if(!voltmx.os.isvaliddate(d1, frmt))
				return false;
...
/**@Deprecated This API converts a given date string to the target format and returns the formatted date. Format will use the tokens dd,mm,yyyy to indicate day, month, and year respectively. The delimiter used between the tokens can be any character. For example, dd/mm/yyyy and (dd)(mm)(yyyy) are valid inputs.*/
	...
	voltmx.os.formatDate = function (d1, sfrmt, tgtfrmt) {
		...
		if(d1 == null || sfrmt == null || tgtfrmt == null) {
	        ...
	        if ((srcfmt == "dd/mm/yyyy" && !voltmx.os.isvaliddate(inputDate, srcfmt)) || datePos == -1 || monthPos == -1 || yearPos == -1 || srcfmt.indexOf("ddd") != -1 || srcfmt.indexOf("mmm") != -1) {
	            return null;
	        }
	        ...
	        targetfmt = targetfmt.replace(/dd/, voltmx.os.padZero(parseInt(dateVal,10)));
	        targetfmt = targetfmt.replace(/mm/, voltmx.os.padZero(parseInt(monthVal,10)));
...
/**@Deprecated This API returns the various date components of a specified date (in a specified format) or the current system date.*/
	...
	voltmx.os.isValidDate = function (date, frmt) {		
		return (arguments.length != 2 || date == null || frmt == null) ? false :voltmx.os.isvaliddate(date, frmt);		
	};
/**@Deprecated This API returns the various date components of a specified date (in a specified format) or the current system date.*/
	//os.dateComponents(date, format);
	//Formats supported:  (dd/mm/yyyy , mm/dd/yyyy, dd/mm/yy , mm/dd/yy) 
	voltmx.os.dateComponents = function (date,frmt){
		...
		if (arguments.length == 0) {
			dateObject = new Date();
				...
				if(!voltmx.os.isvaliddate(date,frmt))
					return null;			 
				dateObject = voltmx.os.getDate(date, frmt);
		...
		if (dateObject) {	
			...
			result["yday"] = voltmx.os.getDayOfYear(day, mon, year);
			result["isdst"] = voltmx.os.checkForDst();
			return result;
		} 
		else
			return null;
	...
	voltmx.os.padZero = function (num) {
	    return num < 10 ? ("0"+num) : num;
	};
/**@Deprecated This API converts a given date string to the target format and returns the formatted date. Format will use the tokens dd,mm,yyyy to indicate day, month, and year respectively. The delimiter used between the tokens can be any character. For example, dd/mm/yyyy and (dd)(mm)(yyyy) are valid inputs.*/	
	voltmx.os.formatdate = function (fmt, dateObj) {
		fmt = fmt.toLowerCase();
	    fmt = fmt.replace(/dd/, voltmx.os.padZero(dateObj.getDate()));
	    fmt = fmt.replace(/mm/, voltmx.os.padZero(dateObj.getMonth() + 1));
...
/**@Deprecated This API returns the various date components of a specified date (in a specified format) or the current system date.*/
    ...
    voltmx.os.isvaliddate = function(dtStr, format){
        var minYear = 1900;
        var maxYear = 2100;
        var dtCh = "/";
        var daysInMonth = voltmx.os.daysArray(12);
        ...
        if (!voltmx.os.isInteger(strMonth) || !voltmx.os.isInteger(strDay) || !voltmx.os.isInteger(strYear) || strMonth.length < 1 || month < 1 || month > 12 || strDay.length < 1 || day < 1 || day > 31 || (month == 2 && day > voltmx.os.daysInFebruary(year)) || day > daysInMonth[month] || strYr.length != 4 || year == 0 || year < minYear || year > maxYear) {
            //alert("invalid date");
            return false
        }
        return true
    ...
    voltmx.os.daysArray = function(n){
        for (var i = 1; i <= n; i++) {
            this[i] = 31
            if (i == 4 || i == 6 || i == 9 || i == 11) {
                this[i] = 30
    ...
    voltmx.os.daysInFebruary = function(year){
        // February has 29 days in any year evenly divisible by four,
        // EXCEPT for centurial years which are not also divisible by 400.
        return (((year % 4 == 0) && ((!(year % 100 == 0)) || (year % 400 == 0))) ? 29 : 28);
    };
	
    voltmx.os.isInteger = function(s){
        var i;
    ...
    voltmx.os.getDate = function(date, format){
        var dateObj;
    ...
    voltmx.os.getDayOfYear = function(day, month, year){
        var i, j;
    ...
    voltmx.os.checkForDst = function(){
